<script lang="ts">
    ...
    const showDialog = (component:any, props: any) => {
        ...
        dialogs = [...dialogs];
        console.debug('当前模块的对话框:', dialogs);
    }

    const handleDialogCloseEvent = (entry, data) => {
        console.debug('关闭对话框', entry);
        const idx = dialogs.indexOf(entry);
        console.debug('当前窗体列表', dialogs, idx);
        if (idx > -1) {
...
<div class="tsui-app-module-page-board-wrapper {className}" {style}>
    <div class:blur={dialogs.length>0} style="width: 100%; height: 100%; position: relative; box-sizing: border-box">
        ...
        {#if pages && pages.length > 0}
            {#each pages as page, i}
                <ModulePage {board} {page} active={i===pages.length} on:close={()=>{pop()}}/>
            {/each}
        {/if}
    </div>
